...
// libc++ (emscripten) only specializes std::char_traits for the standard char
// types, but MyGUI's UString uses std::basic_string<unsigned short> (UTF-16
// data string) and std::basic_string<unsigned int> (UTF-32). Provide the
// missing char_traits specializations. Shared guard with Ogre's identical
// patch so a TU including both headers does not double-define.
#define WASM_CHAR_TRAITS_DEFINED
#include <cstring>
#include <cwchar>
#define MYGUI_DEFINE_CHAR_TRAITS(CT, ITYPE)                                                      \
namespace std {                                                                                  \
    template<> struct char_traits<CT> {                                                          \
        ...
        static int compare(const char_type* a, const char_type* b, size_t n) noexcept {          \
            for (; n; --n, ++a, ++b) { if (*a < *b) return -1; if (*b < *a) return 1; } return 0; } \
        static size_t length(const char_type* s) noexcept { size_t n = 0; while (*s++) ++n; return n; } \
...
MYGUI_DEFINE_CHAR_TRAITS(unsigned short, int)
MYGUI_DEFINE_CHAR_TRAITS(unsigned int, long long)
#undef MYGUI_DEFINE_CHAR_TRAITS
